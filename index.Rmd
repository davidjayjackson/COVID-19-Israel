...
```
```{r}
# US <- subset(df,Reported <="2020-04-25")
summary(df$Cases)
summary(df$Deaths)
df$MA14 <- movingAverage(df$Cases,14)
plot(df$Reported,df$Cases,type="l",main=" COVID-19 Cases Apr. 1,2020 - Aug. 25, 2020",
     xlab="Date Reported",ylab="Cases")
lines(df$Reported,df$MA14,col="red")
grid(col="black")
```
```{r}
# US <- subset(df,Reported >="2020-04-01" & Reported <="2020-08-25")
df$Deaths14 <- movingAverage(df$Deaths,14)
plot(df$Reported,df$Deaths,type="l",main="COVID-19 Deaths Apr. 1,2020 - Aug. 25, 2020",
