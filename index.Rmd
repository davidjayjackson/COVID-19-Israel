...
```{r}
US <- subset(df,Reported >="2020-07-01")
...
```{r}
A <- subset(US,Reported >="2020-04-01")
ggplot(A) + geom_line(aes(x=Reported,y=Rate)) +
  scale_y_continuous(labels = scales::percent) +
  labs(title="Israel COVID19 Mortality Rate ",x="Date Reported",y="Mortality Rate") +
  geom_hline(yintercept = mean(A$Rate),col="red") +
  geom_smooth(aes(x=Reported,y=Rate,col="Rate"),span=0.25)
...
ggplot(US) + geom_line(aes(x=Reported,y=Deaths,col="Daily Deaths")) +
  labs(title="COVID-19 Deaths by Date") + ylim(0,75) +
...
ggplot(USA) + geom_line(aes(x=Reported,y=Deaths,col="Daily Deaths")) +
  labs(title="COVID-19 Deaths by Date (since Jun. 1, 2020)") + ylim(0,75) +
  geom_smooth(aes(x=Reported,y=Deaths,col="Loess"),span=0.25)
```
