...
vac <- vac %>%select(date,location,iso_code,new_vaccinations,
                    ...
                    na.omit()
summary(vac$new_vaccinations)
```

```{r}
p5 <- ggplot(vac) + geom_line(aes(x=date,y=new_vaccinations)) +
  scale_y_continuous(labels=comma) +
  labs(title="Israel s New Vaccinations By Date") +
...
p6 <- ggplot(vac) + geom_line(aes(x=date,y=total_vaccinations)) +
  scale_y_continuous(labels=comma) +
  labs(title="Irael's Total Vaccinations By Date") 
ggplotly(p6)
```

```{r}
vac$Rate <- vac$total_vaccinations/vac$population
p7 <- ggplot(vac) + geom_line(aes(x=date,y=Rate)) + 
    scale_y_continuous(labels=percent)  +
  labs(title="Israel's vaccination Rate By Day") 
ggplotly(p7)
