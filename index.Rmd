...
```{r}
vac <- dbGetQuery(db,"select * from OWID where location = 'Israel'")
...
vac <- vac %>%select(date,location,iso_code,new_vaccinations,
                    ...
                    na.omit()
summary(vac$speople_vaccinated,na.rm=TRUE)
summary(vac$people_fully_vaccinated,na.rm=TRUE)
```
...
p5 <- ggplot(vac) + geom_line(aes(x=date,y=new_vaccinations)) +
  ...
  labs(title="Israel's Daily Doses By Date") 
...
p6 <- ggplot(vac) + geom_line(aes(x=date,y=total_vaccinations)) +
  scale_y_continuous(labels=comma) +
  labs(title="Israel's Total (Full) Doses Date") 
ggplotly(p6)
```
...
p5 <- ggplot(vac) + geom_line(aes(x=date,y=people_vaccinated)) +
  scale_y_continuous(labels=comma) +
  labs(title="Israel's Number of People Vaccinatations By Date") 
...
p6 <- ggplot(vac) + geom_line(aes(x=date,y=people_fully_vaccinated)) +
  scale_y_continuous(labels=comma) +
  labs(title="Israel's Total  Number of People Fully Vaccinated By Date") 
ggplotly(p6)
